...
tf.flags.DEFINE_integer(
    "num_batches",
    intfromenv("EXEC_ITER_NUMBER", 20),
...
tf.flags.DEFINE_integer(
    "autotune_threshold", None, "The autotune threshold for the models"
)
...
tf.flags.DEFINE_string(
    "optimizer", "sgd", "Optimizer to use: momentum or sgd or rmsprop"
)
tf.flags.DEFINE_float("learning_rate", None, """Initial learning rate for training.""")
tf.flags.DEFINE_float(
    "num_epochs_per_decay", 0, """Steps after which learning rate decays."""
)
tf.flags.DEFINE_float(
...
tf.flags.DEFINE_float("weight_decay", 0.00004, """Weight decay factor for training.""")
...
tf.flags.DEFINE_boolean(
    "use_nccl", True, "Whether to use nccl all-reduce primitives where possible"
)

# Distributed training flags.
tf.flags.DEFINE_string(
    "job_name", "", 'One of "ps", "worker", "".  Empty for local training'
)
...
tf.flags.DEFINE_string(
    "pretrain_dir", None, """Path to pretrained session checkpoints."""
)
tf.flags.DEFINE_string(
    "result_storage",
    None,
...
class ConvNetBuilder(object):
    ...
    def affine(
        ...
        with tf.variable_scope(name):
            ...
            kernel = tf.get_variable(
                "weights",
                [num_channels_in, num_out_channels],
                self.data_type,
    ...
    def resnet_bottleneck_v1(
        self, depth, depth_bottleneck, stride, input_layer=None, in_size=None
    ):
        if input_layer is None:
            input_layer = self.top_layer
        if in_size is None:
...
class BenchmarkCNN(object):
    ...
    def __init__(self):
        ...
        autotune_threshold = (
            FLAGS.autotune_threshold if (FLAGS.autotune_threshold) else 1
        )
        min_autotune_warmup = 5 * autotune_threshold * autotune_threshold
        self.num_warmup_batches = (
            FLAGS.num_warmup_batches
        ...
        if FLAGS.variable_update == "parameter_server":
            if self.job_name:
                if not FLAGS.staged_vars:
                    ...
                    self.variable_mgr = variable_mgr.VariableMgrDistributedFetchFromStagedPS(
                        self
                    )
            else:
                if not FLAGS.staged_vars:
    ...
    def _benchmark_cnn(self):
        ...
        if self.job_name and FLAGS.cross_replica_sync:
            ...
            fetches.append(
                self.add_sync_queues_and_barrier(
        ...
        if (
            is_chief
            and FLAGS.summary_verbosity
            and FLAGS.train_dir
            and FLAGS.save_summaries_steps > 0
        ):
            summary_writer = tf.summary.FileWriter(
                FLAGS.train_dir, tf.get_default_graph()
            )
        ...
        with sv.managed_session(
            ...
            config=create_config_proto(),
            start_standard_services=FLAGS.summary_verbosity > 0,
        ) as sess:
            for i in xrange(len(enqueue_ops)):
                sess.run(enqueue_ops[: (i + 1)])
            sess.run(local_var_init_op)
            ...
            global_step_watcher = GlobalStepWatcher(
                ...
                len(self.worker_hosts) * (self.num_warmup_batches + self.num_batches)
                - 1,
            )
            ...
            while not done_fn():
                ...
                if FLAGS.rand_delay:
                    time.sleep(
                        randint(100, 500) / 1000.0
                    )  # delay randomly from 100ms to 500 ms
    ...
    def _build_model(self):
        ...
        for device_num in range(len(self.devices)):
            with self.variable_mgr.create_outer_variable_scope(
                device_num
            ), tf.name_scope("tower_%i" % device_num) as name_scope:
                results = self.add_forward_pass_and_gradients(
                    images_splits[device_num],
                    labels_splits[device_num],
                    nclass,
                    phase_train,
                    device_num,
                    input_data_type,
